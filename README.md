...
* `shutdown_timeout` - задать время ожидания завершения всех текущих соединений при получении сервером сигнала `shutdown`. Изменяя это значение принимайте во внимание опции `request_timeout` и `max_keepalive` в `protocol_opts` для достижения желаемого эффекта.
